...
final class SMCKitTests: XCTestCase {
    ...
    func testFanCurveInterpolation_emptyPoints() {
        let curve = FanCurve(points: [])
        XCTAssertEqual(curve.targetPercentage(for: 70), 0.0, "Empty curve (System mode) should return 0.0")
    }
    ...
    func testSystemProfileHasNoControlPoints() {
        XCTAssertTrue(FanProfile.system.curve.points.isEmpty, "System should have no curve points — Apple stays in control")
    }
